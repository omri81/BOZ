...
class TakeVC: UIViewController{
    ...
    func readFromDefault() {
        ...
        if let  mail = prefs.string(forKey: DONATOR_EMAIL){
    ...
    func storeDefault(){
        ...
        prefs.set(textInput.email, forKey: DONATOR_EMAIL)
    ...
    private func getTexts() -> (firstName: String?, lastName:String?,phone:String?, address:String?, email:String?) {
        return (firstName: firstName.text, lastName: lastName.text, phone : phoneNumber.text, address: self.addressTF.text, email: email.text)
    }
